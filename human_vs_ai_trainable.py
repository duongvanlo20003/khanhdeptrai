...
# Hàm xử lý input từ người chơi
def handle_player_input(player, event):
    if event.type == pygame.KEYDOWN:
        if event.key == player.controls['left']:
            player.move(-1)
        elif event.key == player.controls['right']:
            player.move(1)
        elif event.key == player.controls['jump']:
            player.jump()
        elif event.key == player.controls['shoot']:
            player.shoot(bullets)
    elif event.type == pygame.KEYUP:
        if event.key == player.controls['left'] or event.key == player.controls['right']:
            player.move(0)
...
def main_menu():
    ...
    while menu_running:
        screen.fill(BLACK)
        title = font.render("Gun Mayhem Clone", True, WHITE)
        screen.blit(title, (WIDTH // 2 - title.get_width() // 2, HEIGHT // 4))
        ai_vs_ai_text = font.render("1. AI vs AI", True, WHITE)
        screen.blit(ai_vs_ai_text, (WIDTH // 2 - ai_vs_ai_text.get_width() // 2, HEIGHT // 2))
        player_vs_ai_text = font.render("2. Player vs AI", True, WHITE)
        screen.blit(player_vs_ai_text, (WIDTH // 2 - player_vs_ai_text.get_width() // 2, HEIGHT // 2 + 50))
        pygame.display.flip()
...
def reset_game(mode='ai_vs_ai'):
    ...
    if mode == 'ai_vs_ai':
        ...
        player1 = Player(300, 100, WHITE, controls={'left': pygame.K_a, 'right': pygame.K_d, 'jump': pygame.K_w, 'shoot': pygame.K_SPACE})
        player2 = AIBot(500, 100, GREEN)
        player2.ai.load_q_table("q_table_player2.pkl")
    players.add(player1, player2)
    return player1, player2

# Khởi tạo lịch sử điểm số
    ...
    while not episode_ended:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
                episode_ended = True
            if mode == 'player_vs_ai' and not player1.is_ai:
                handle_player_input(player1, event)

        # Get states and perform actions
        if mode == 'ai_vs_ai':
        ...
        # Get next states and update Q-values
        if mode == 'ai_vs_ai':
